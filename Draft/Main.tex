...
Under the IEM, element-wise sample mean of the adjacency matrices $\bar{A}$ is the MLE for the mean graph $P$.
It is an unbiased with entry-wise variance $\mathrm{Var}(\bar{A}_{ij}) = P_{ij} (1-P_{ij})/M$. Moreover, $\bar{A}$ is the uniformly minimum-variance unbiased estimator, so it has the smallest variance among all unbiased estimators.
