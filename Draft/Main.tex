...
As further evidence for the improvement using $\hat{P}$, even when the dimension must be derived from data, Table~\ref{tab:corr_re}.
For each atlas and each sample size we compare the \citet{zhu2006automatic} with the USVT method and note that both perform well, especially for large $N$ and small $M$.
...
\multicolumn{4}{c}{JHU} \\\midrule
        & $m=1$  & $m=5$  & $m=10$ \\
ZG      & 0.54 & 0.81 & 1.14 \\
USVT    & 0.56 & 0.84 & 1.12 \\\midrule
\multicolumn{4}{c}{Desikan} \\ \midrule
        & $m=1$  & $m=5$  & $m=10$ \\
ZG      & 0.52 & 0.81 & 1.14 \\
USVT    & 0.50 & 0.81 & 1.09 \\\midrule
\multicolumn{4}{c}{CPAC200} \\\midrule
        & $m=1$  & $m=5$  & $m=10$ \\
ZG      & 0.51 & 0.68 & 0.89 \\
