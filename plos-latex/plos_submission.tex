...
\bibitem{stein1956inadmissibility}
Stein C.
...
\bibitem{ginestet2014hypothesis}
...
\bibitem{scheinerman2010modeling}
...
\bibitem{durante2014nonparametric}
...
Here we present the proofs of the results in Section~\ref{section:theoretical_result}. To keep the ideas clear and concise, we leave {\color{red} RT: present tense?} out some details which are only slight changes to previous works.
...
We observe {\color{red} RT: present tense?} $M$ graphs on $N$ vertices $A^{(1)}, \cdots, A^{(M)}$ sampled independently from the SBM conditioned on $\tau$.
