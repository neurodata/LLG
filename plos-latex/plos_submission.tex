...
As with real valued data, one may want to define the mean of a population of graphs to itself {\color{red} Dan: not sure about ``to itself'' here} be a graph such as for the median graph \citep{jiang2001median}.
...
The latent positions model (LPM) proposed by Hoff et. al. (2002) \citep{hoff2002latent} captures such structure, where each vertex is associated with a latent position that influences the adjacencies for that vertex.
In this model, each vertex $i$ has an associated latent vector $X_i \in \Re^d$.
Based on the latent positions, the existence of edges are {\color{red} Dan: are or is?} conditionally independent with {\color{red} Dan: maybe we don't need to add ``a'' here?} probability that only depends on the latent vectors of the incident vertices through a link function. If $d$ is much smaller than the number of vertices $N$ and the link function is known, LPMs are more parsimonious models compared to IEM, requiring only $dN$ parameters rather than $\binom{N}{2}$.
...
Similarly, the magnitude of the latent position encodes the vertices overall tendency to form edges, with larger magnitudes leading to more edges. {\color{red} Dan: Not sure about this one}
...
\]
In the RDPG model, each vertex $i$ is associated with latent position $X_i$, and conditioned on the latent positions $X$, {\color{red} Dan: I don't think we need ``and'' here} the edges $A_{ij}$ are distributed independently $ \text{Bern}(\left \langle X_i, X_j \right \rangle)$ for $i<j$. {\color{red} Dan: Not sure about this one}
...
To compute our estimator $\hat{P}$, we need to additionally {\color{red} Dan: do we need to change it to ``also''?} specify what rank $d$ to use and there are various ways of dealing with dimension selection. 
...
To compensate for this issue, we use an iterative method developed by \citet{scheinerman2010modeling}. 
...
\]
\end{lemma}
The first part of this Lemma gives the form of the asymptotic variance of $\hat{P}$ and the second part ensures that additionally the estimator is approximately unbiased for $P$. {\color{red} Dan: Is it okay now?}
...
To illustrate Eq.~\ref{eq:approx_re} of Theorem~\ref{thm:ARE}, Fig.~\ref{fig:RErho} shows $2/\rho_1$ and $1/\rho_1+1/\rho_2$, the scaled asymptotic RE for pairs of vertices with {\color{red} Dan: in?} block one and pairs of vertices in different blocks, respectively, in a  two-block stochastic blockmodel.
...
\begin{proposition}
Suppose that $A^{(1)},A^{(2)},\dotsc,A^{(M)}$ are distributed iid from an RDPG distribution with common latent positions $X_1,\dotsc,X_n$, which are distributed iid from some distribution. {\color{red} Dan: Not sure about ``distributed iid''}
...
To investigate the finite sample relative efficiency, we first sample 1000 Monte Carlo replicates from the above SBM distribution with different numbers of vertices $N$ and a fixed number of graphs $M$. The  relative efficiency $\mathrm{RE}(\bar{A}_{ij}, \hat{P}_{ij})$ can be estimated since $P$ is known for this simulation. Since the relative efficiency only depends on the blocks {\color{red} Dan: I think it is with ``s''} memberships of the pair $i,j$, we estimate the relative efficiency for each block pair using
\[
...
The dataset contains 454 different brain scans, each of which was processed to yield an undirected, unweighted graph with no self-loops, using the pipeline describe in \citet{gray2013migraine} and \citet{kiar2016m2g}. {\color{red} Dan: Seems that the format is like this? Do we need to add the year?}
...
For each atlas and each sample size, we compare the \citet{zhu2006automatic} with the USVT method and note that both perform reasonably well relative to the full-dimensional $\bar{A}$.
...
in Fig.~\ref{fig:Diff_between_desikan} we plot the 50 edges with the largest differences $|\bar{A} - P| - |\hat{P} - P|$ {\color{red} Dan: Do we need to add subscription $ij$ here?} according to the location of the corresponding regions in the brain. Red edges indicate that $\hat{P}$ overestimates $P$, while blue means that $\hat{P}$ underestimates $P$. The edge width is determined by the estimation error for $\hat{P}$, where pairs with larger estimation error are represented by thicker lines.
...
Note that $\hat{P}$ appears to better estimate the true probabilities {\color{red} Dan: probability or probabilities?} $P$, especially for edges between the two hemispheres, in the upper right (and lower left) block. {\color{red} Dan: I think parenthesis is good here}
}
...
When $M$ is small, $\hat{P}$ outperforms $\bar{A}$ with a flexible range of the embedding dimension including those selected by the Zhu and Ghodsi method while {\color{red} Dan: I don't like ``however''} when $M$ is large enough, both estimators perform well with the decision between the two being less conclusive.
...
In particular, we show that for a stochastic blockmodel with fixed number of blocks $K$, block size proportion $\rho$, and number of graphs $M$, the low-rank estimator $\hat{P}$ has MSE which is on the {\color{red} Dan: do we need to change it to``an''?} order of $N$ times lower than the MSE for $\bar{A}$.
...
For this reason, we use the 3rd elbow in the experiments performed in {\color{red} Dan: do wee need to change it to ``for''?} this work.
...
On the other hand, we have observed that in our setting, many dimensions will yield nearly optimal mean squared errors. Thus efforts to ensure the selected dimension is in the appropriate range are more important than finding the best dimension. {\color{red} Dan: I have changed this sentence a little bit}
...
Denote matrix of between {\color{red} Dan: in-between?} block edge probabilities by $B = \nu \nu^T\in[0,1]^{K\times K}$ which we assume has rank $K$ and is positive definite. 
...
The proofs presented here will rely on a central limit theorem developed in {\color{red} Dan: ``in'' instead of ``by''?} \citet{athreya2013limit}. 
